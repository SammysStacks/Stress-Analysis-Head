...
class wesadInterface(globalMetaAnalysis):
    ...
    def getData(self, showPlots = False):
        ...
        allSynchronizedData = []; subjectOrder = []
        allContextualInfo = []
        ...
        for subjectFolderName in natsorted(os.listdir(self.subjectFolders)):
            ...
            if os.path.isdir(subjectFolder) and self.universalMethods.isNumber(subjectFolderName[1:]):
                ...
                # Extract the subject demographic infomration
                ...
                # Load in synchornized data (compiled by researchers).
                with open(synchronizedDataFile, 'rb') as file:
                    synchronizedData = pickle.load(file, encoding='latin1')
                
                startTimes = []; endTimes = []
                sssqAnswers = []; dimAnswers = []
                panasAnswers = []; staiAnswers = []
                ...
                for row in labelInfoRead:
                    ...
                    if len(compiledNumbers) == 26:
                        ...
                        compiledNumbers.pop(); compiledNumbers.pop();
                        compiledNumbers.append(sadAnswer)
                    
                    # Save the questionaire data
                ...
                # Assert that we have all the questionaire data.
    ...
    def calculateOffsetTime(self, timepoints, classLabels, experimentTimes):
...
if __name__ == "__main__":
    ...
    if analyzingData:        
        ...
        allSynchronizedData, subjectOrder, allExperimentalTimes, allExperimentalNames, \
            allSurveyAnswerTimes, allSurveyAnswersList, allContextualInfo = wesadAnalysisClass.getData(showPlots = True)
        allCompiledDatas = wesadAnalysisClass.compileAllData(allSynchronizedData, allExperimentalTimes, showPlots = False)
        # Compile the data: specific to the device worn.
        streamingOrder, biomarkerOrder, featureAverageWindows, filteringOrders = wesadAnalysisClass.getStreamingInfo()
        # Analyze and save the metadata features
        wesadAnalysisClass.extractFeatures(allCompiledDatas, subjectOrder, allExperimentalTimes, allExperimentalNames, allSurveyAnswerTimes, allSurveyAnswersList, allContextualInfo, 
                                           streamingOrder, biomarkerOrder, featureAverageWindows, filteringOrders, interfaceType = 'wesad', reanalyzeData = True, showPlots = True)
            
    if trainingData:
        # Prepare the data to go through the training interface.
        streamingOrder, biomarkerOrder, featureAverageWindows, biomarkerFeatureNames = wesadAnalysisClass.compileTrainingInfo()
        ...
        allRawFeatureTimesHolders, allRawFeatureHolders, allRawFeatureIntervals, allRawFeatureIntervalTimes, \
            allAlignedFeatureTimes, allAlignedFeatureHolder, allAlignedFeatureIntervals, allAlignedFeatureIntervalTimes, subjectOrder, \
                experimentOrder, activityNames, activityLabels, allFinalFeatures, allFinalLabels, featureLabelTypes, surveyQuestions, surveyAnswersList, surveyAnswerTimes \
                    = wesadAnalysisClass.trainingProtocolInterface(streamingOrder, biomarkerOrder, featureAverageWindows, biomarkerFeatureNames, plotTrainingData, metaTraining=True)
