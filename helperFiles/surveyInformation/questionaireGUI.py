...
import sys
import itertools
# Modules for storing questionnaire
import json
# GUI Modules
...
class stressQuestionnaireGUI(QtWidgets.QMainWindow):
    ...
    def __init__(self, readData=None, folderPath="./"):
        ...
        self.subjectInformationQuestions = None
        self.startSurveyTime = None
        self.radioButtons = None
        self.currentExperimentName = None
        ...
        # Add a link to the streaming class
        ...
        self.setWindowTitle("Stress Questionnaire App")
        ...
        self.guiApp.exec()  # Run the GUI continuously
    ...
    def surveyDemographicOptions(self, subjectInfoFile):
        ...
        with open(subjectInfoFile) as questionnaireFile:
            data = json.load(questionnaireFile)
        ...
        # Each question has its own answer choices, and can have multiple answers. Ex. Question = height, answers = feet, inches
    ...
    # ---------------------------------------------------------------------- #
    # -------------- Backend Integration with Streaming Data -------------- #
    ...
    def _recordSurveyResults(self, surveyAnswers, surveyInd):
        # Assert the validity of the incoming data
        ...
        # Print the answers if testing.
            ...
            for questionsInd in range(len(self.surveyQuestions[surveyInd])):
                print("\t", self.surveyQuestions[surveyInd][questionsInd] + ":", self.surveyAnswerChoices[surveyInd][surveyAnswers[questionsInd] - 1])
    ...
    def _recordSubjectInfo(self, answerBoxes, metricChecks={}):
        ...
        tempQuestionIndex = 0
        tempAnswerIndex = 0
        # formats answers to question based on answer box type
        ...
        for answerBoxIndex in range(len(answerBoxes)):
            ...
            if answerBoxes[answerBoxIndex][1] == "Dropdown":
                ...
                if answerBoxes[answerBoxIndex][0].text():
                    ...
                    # checks that there is an inputted answer to the answer box
                    if "Height" in self.subjectInformationQuestions[tempQuestionIndex]:
                        heightInfo.append(0)
                    else:
                ...
                # answer box is not of Dropdown or textbox format, which is invalid
                self.informBadInfoSurvey()
                return

        # reformat height information to fit the subject answer format.
        ...
        # if foundGoodData:
        # Is you are streaming in data, then send the data to the streaming class.
        if self.readData is not None:
    ...
    def setButtonAesthetics(widget, backgroundColor="black", textColor="white", fontFamily="Arial", fontSize=15):
        ...
        widget.setStyleSheet(
            "background-color: " + backgroundColor + "; " + "color: " + textColor + "; " + "border: 0.5px solid black; font-weight: 5rem; padding: 3rem 5rem; text-align: center; width: 100%; margin: 1rem 2rem;")

        # Add font information
        widget.setFont(QFont(fontFamily, fontSize))

    @staticmethod
    def setDimensions(element, minWidth, maxWidth, minHeight, maxHeight):
        """
        This method sets the minimum and maximum dimensions of a given widget.
        """
        # Add dynamic sizing
        if minHeight is not None and minWidth is not None:
    ...
    def addTitleToGui(self, titleText, fontSize=50, backgroundColor="white"):
        ...
        titleLabel.setStyleSheet("background-color: " + backgroundColor + "; " + "color: black; text-align: center; margin: 1rem auto;")
    ...
    def inputText(self, startRow, startColumn, rows, columns, placeholderText="Enter Text Here", onlyInt=False):
        """
        This method creates and returns an input textbox Widget with default
        styling given position and dimensions.
        
        If only an integer, the widget will only accept integer values.
    ...
    def displayHomeScreen(self):
        ...
        """
        # Start off fresh by cleaning the GUI elements.
        self.clearScreen()
        ...
        the experiment screen. The second one is the add subject information 
        ...
        quitSurveyButton.clicked.connect(self.finishedRun)
        # Add button aesthetics: colors and text styles.
    ...
    @staticmethod
    def metricCheckbox_Height(checkbox, box1, box2, placeholder):
    ...
    @staticmethod
    def metricCheckbox_Weight(checkbox, box, placeholder):
        if checkbox.isChecked():
    ...
    def displayExperimentScreen(self):
        ...
        if self.currentExperimentName != "":
            ...
            # Add the start survey button to the layout.
    ...
    def displaySurveyQuestions(self, startRow, surveyInd):
        ...
        five buttons in a radio button group.
        """
        blankSpace = QtWidgets.QLabel(" ")
    ...
    def display1RowSurveyButtons(self, startRow, highlightRow, surveyInd):
        ...
        for answerChoiceInd in range(len(curr_answerChoices)):
            ...
            buttonGroupRow.addButton(newButton)  # Specify that this button is connected to the other ones on the row.
    ...
    def saveSurveyResults(self, questionObjects, surveyInd):
        ...
        self._recordSurveyResults(surveyAnswers, surveyInd)
...
if __name__ == "__main__":
    ...
    guiObject = stressQuestionnaireGUI(readData=None, folderPath="./")

    # import threading
    # threading.Thread(target = stressQuestionnaireGUI, args = (readData, stressQuestionnaireFile), daemon=False).start()
    # print("THANK YOU :)")
    # while True:
    #     infiniteLoop = "Yes"
