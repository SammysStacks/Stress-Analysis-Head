...
class generalAutoencoderBase(_autoencoderModules.autoencoderModules):
    def __init__(self, accelerator, compressionFactor, expansionFactor):
        ...
        self.compressDataCNN_preprocessing = self.signalEncodingModule_preprocess(inChannel = 1, channelIncrease = 4)
        self.expandDataCNN_preprocessing = self.signalEncodingModule_preprocess(inChannel = 1, channelIncrease = 4)
        # Autoencoder modules for postprocessing.
        self.compressDataCNN_postprocessing = self.signalEncodingModule_postprocess(inChannel = 1, channelIncrease = 4)
        self.expandDataCNN_postprocessing = self.signalEncodingModule_postprocess(inChannel = 1, channelIncrease = 4)
    ...
    def signalEncodingModule_preprocess(self, inChannel = 1, channelIncrease = 4):
        return nn.Sequential(
            ...
            self.convolutionalThreeFilters_resNetBlock(numChannels = [inChannel, 8*inChannel, 8*inChannel, inChannel], kernel_sizes = [3, 3, 3], dilations = [1, 1, 1], groups = [1, 1, 1]),
            
            # Convolution architecture: channel expansion
            self.convolutionalThreeFilters(numChannels = [inChannel, 2*inChannel, 4*inChannel, 4*inChannel], kernel_sizes = [3, 3, 3], dilations = [1, 1, 1], groups = [1, 1, 1]),
            
            # Convolution architecture: feature engineering
            self.convolutionalThreeFiltersBlock(numChannels = [4*inChannel, 8*inChannel, 8*inChannel, 4*inChannel], kernel_sizes = [3, 3, 3], dilations = [1, 1, 1], groups = [1, 1, 1]),
    
            # Convolution architecture: channel compression
            self.convolutionalThreeFilters(numChannels = [4*inChannel, 4*inChannel, 2*inChannel, inChannel], kernel_sizes = [3, 3, 3], dilations = [1, 1, 1], groups = [1, 1, 1]),
    
            # Convolution architecture: feature engineering
            self.convolutionalThreeFiltersBlock(numChannels = [inChannel, 4*inChannel, 4*inChannel, inChannel], kernel_sizes = [3, 3, 3], dilations = [1, 1, 1], groups = [1, 1, 1]),
        )
    ...
    def ladderModules(self, inChannel = 1, channelIncrease = 4):     
        return nn.Sequential( 
            # Convolution architecture: signal-specific feature engineering
            self.convolutionalThreeFilters_noActivationResNetBlock(numChannels = [inChannel, 8*inChannel, 8*inChannel, inChannel], kernel_sizes = [3, 3, 3], dilations = [1, 1, 1], groups = [1, 1, 1]),    
            
            # Convolution architecture: channel expansion
            self.convolutionalThreeFilters_noActivation(numChannels = [inChannel, 2*inChannel, 4*inChannel, 4*inChannel], kernel_sizes = [3, 3, 3], dilations = [1, 1, 1], groups = [1, 1, 1]),
            
            # Convolution architecture: feature engineering
            self.convolutionalThreeFilters_noActivationBlock(numChannels = [4*inChannel, 8*inChannel, 8*inChannel, 4*inChannel], kernel_sizes = [3, 3, 3], dilations = [1, 1, 1], groups = [1, 1, 1]),
            
            # Convolution architecture: channel compression
            self.convolutionalThreeFilters_noActivation(numChannels = [4*inChannel, 4*inChannel, 2*inChannel, inChannel], kernel_sizes = [3, 3, 3], dilations = [1, 1, 1], groups = [1, 1, 1]),

            # Convolution architecture: feature engineering
            self.convolutionalThreeFilters_noActivationBlock(numChannels = [inChannel, 4*inChannel, 4*inChannel, inChannel], kernel_sizes = [3, 3, 3], dilations = [1, 1, 1], groups = [1, 1, 1]),
        )
    ...
    def expansionAlgorithm(self, compressedData, nextSequenceLength, initialSequenceLength):  
        ...
        processedData = self.expandDataCNN_preprocessing(processedData) + processedData
        ...
        processedData = self.expandDataCNN_postprocessing(processedData) + processedData
    ...
    def compressionAlgorithm(self, expandedData, nextSequenceLength, initialSequenceLength): 
        ...
        processedData = self.compressDataCNN_preprocessing(processedData) + processedData
        
        # Convolution architecture: change signal's dimension.
        processedData = nn.functional.interpolate(processedData, size=nextSequenceLength, mode='linear', align_corners=True, antialias=False)
        
        # Process the reduced data.
        processedData = self.compressDataCNN_postprocessing(processedData) + processedData
        encodedData = self.loweringOperator(processedData, initialSequenceLength)
    ...
    def calculateEncodingLoss(self, originalData, encodedData, initialSequenceLength):
        ...
        decodedData = self.dataAugmentation.addNoise(encodedData, trainingFlag = True, noiseSTD = 0.005)
