...
class signalEncoderBase(_convolutionalHelpers.convolutionalHelpers):
    def __init__(self, signalDimension = 64, encodedSamplingFreq = 4, plotSignalEncoding = False):
        ...
        self.initialTransformation = self.subspaceTransformation(inChannel = 1, channelIncrease = 32, numGroups = 1)
        self.finalTransformation = self.subspaceTransformation(inChannel = 1, channelIncrease = 32, numGroups = 1)
        ...
        self.compressChannels = self.signalEncodingModule(inChannel = self.encodedSamplingFreq, outChannel = self.numCompressedSignals, channelIncrease = 4)
        self.compressChannelsPCA = self.signalEncodingModule(inChannel = self.numCompressedSignals, outChannel = self.numCompressedSignals, channelIncrease = 4)
        
        # Create model for learning complex information.
        self.expandChannelsANN = self.signalEncodinANN(inputDimension = self.signalDimension, scaleFactor = 1)
        self.embedComponents = self.signalEncodinANN(inputDimension = self.encodedSamplingFreq*self.numCompressedSignals, scaleFactor = 2)
        
        # Delta learning modules to predict the residuals.
        # self.deltaCompression_channelInfo = self.channelSpecificModule(inChannel = 1, outChannel = self.numCompressedSignals, channelIncrease = 16, numGroups = self.encodedSamplingFreq)
        # self.deltaExpansion_channelInfo = self.channelSpecificModule(inChannel = 1, outChannel = self.encodedSamplingFreq, channelIncrease = 16, numGroups = self.numCompressedSignals)
        ...
        if plotSignalEncoding:
            plt.plot(self.channelEncoding.detach().cpu().T)
            plt.show()
    ...
    def signalCompression(self, expandedData):
        ...
        estimatedSignalArrangement = self.compressChannels(expandedData)
        
        # Perform the optimal compression via PCA and embed channel information (for reconstruction).
        pcaProjection, principal_components = self.pcaCompression(expandedData, self.numCompressedSignals, False)
        ...
        # reconstructed = (reconstructed * signals_std) + signals_mean
        # print((reconstructed - expandedData).pow(2).mean(dim=-1).mean(dim=1).mean())
        # sys.exit()
                
        # Crude estimation of how the channels will compress.
        # potentialSignalArrangement = (expandedData[:, :-1, :] + expandedData[:, 1:, :])/2
        averagePrediction = self.downsampleChannels(expandedData.transpose(1, 2)).transpose(1, 2)
                
        # Return the summation of each prediction.
        return estimatedSignalArrangement + averagePrediction
    ...
    def signalExpansion(self, compressedData):        
        ...
        # Learn the positional encoding through an ANN.
        compressedData = self.expandChannelsANN(compressedData)
    ...
    def pcaCompression(self, signals, numComponents, standardizeSignals = False):
    ...
    def embedChannelInfo(self, projected_signals, principal_components):
        ...
        # Flatten the principle component information.
        principal_components = principal_components.view(batchSize, encodedSamplingFreq*nunComponents)
        
        # Flatten the principle component information.
        principal_components = self.embedComponents(principal_components)
        
        # Reshape the principle component information.
        principal_components = principal_components.view(batchSize, encodedSamplingFreq, nunComponents)
    ...
    def signalEncodinANN(self, inputDimension, scaleFactor = 2):        
        return _convolutionalHelpers.ResNet(
                    module = nn.Sequential( 
                        # Neural architecture: Layer 1.
                        nn.Linear(inputDimension, inputDimension*scaleFactor, bias=True),
                        nn.SELU(),
                        
                        # Neural architecture: Layer 2.
                        nn.Linear(inputDimension*scaleFactor, inputDimension, bias=True),
                        nn.SELU(),
                ), numCycles = 1)
    ...
    def subspaceTransformation(self, inChannel = 1, channelIncrease = 8, numGroups = 1):
        # Setup the problem for each group.
        inChannel = int(inChannel*numGroups)
        ...
        return nn.Sequential( 
                ...
                # Convolution architecture: feature engineering
                self.deconvolutionalFilter_resNet(numChannels = [inChannel, inChannel, inChannel], kernel_sizes = [3, 3], dilations = [1, 1], groups = [1, 1]),
        )
    ...
    def calculateEncodingLoss(self, startingData, originalData, encodedData, numSignalPath, compressingDataFlag): 
        ...
        decodedStartingData = encodedData.clone()
        ...
        for pathInd in range(len(numSignalPath)-1, -1, -1):
            ...
            decodedStartingData, _, _ = self.forward(decodedStartingData, targetNumSignals = numSignalPath[pathInd], signalEncodingLayerLoss = 0, calculateLoss = False)
        ...
        squaredErrorLoss_original = (startingData - decodedStartingData).pow(2).mean(dim=-1).mean(dim=1)
        ...
        normalizationLayerLoss = normalizationLayerLoss + self.calculateStandardizationLoss(decodedData, expectedMean = 0, expectedStandardDeviation = 1, dim=-1)
        normalizationLayerLoss = normalizationLayerLoss + self.calculateStandardizationLoss(decodedEncodedOriginalData, expectedMean = 0, expectedStandardDeviation = 1, dim=-1)
        normalizationLayerLoss = normalizationLayerLoss + self.calculateStandardizationLoss(encodedDecodedOriginalData, expectedMean = 0, expectedStandardDeviation = 1, dim=-1)
                
        print(squaredErrorLoss_original.mean().item(), squaredErrorLoss_forward.mean().item(), squaredErrorLoss_middle.mean().item(), 
              squaredErrorLoss_backward.mean().item(), normalizationLayerLoss.mean().item())
        ...
        return (squaredErrorLoss_forward + squaredErrorLoss_backward + squaredErrorLoss_middle + 4*squaredErrorLoss_original + normalizationLayerLoss)/4
    
    def updateLossValues(self, startingData, originalData, encodedData, numSignalPath, compressingDataFlag, signalEncodingLayerLoss):
        # Keep tracking of the loss through each loop.
        layerLoss = self.calculateEncodingLoss(startingData, originalData, encodedData, numSignalPath, compressingDataFlag)
...
class signalEncoding(signalEncoderBase):
    def __init__(self, signalDimension = 64, encodedSamplingFreq = 5):
    ...
    def forward(self, signalData, targetNumSignals = 64, signalEncodingLayerLoss = 0, calculateLoss = True):
        ...
        while targetNumSignals < signalData.size(1):
            ...
            if calculateLoss: signalEncodingLayerLoss = self.updateLossValues(startingData, originalData, signalData, numSignalPath, True, signalEncodingLayerLoss)
        ...
        while signalData.size(1) < targetNumSignals:
            ...
            # Aggregate all the layer loss values together.
            if calculateLoss: signalEncodingLayerLoss = self.updateLossValues(startingData, originalData, signalData, numSignalPath, False, signalEncodingLayerLoss)
