...
class transformerLayer(nn.Module):
    def __init__(self, signalEmbeddingDim, num_heads, numLayers):
        super(transformerLayer, self).__init__()  
        # General model parameters.
        self.signalEmbeddingDim = signalEmbeddingDim
        self.numLayers = numLayers
        self.num_heads = num_heads
        
        # Define the attention mechanism.
        self.attentionMechanism = nn.MultiheadAttention(
            embed_dim = signalEmbeddingDim, # The dimension of each index of the signal.
            ...
            dropout = 0.3,          # Dropout probability on attn_output_weights. Default: 0.0 (no dropout).
            bias = True,            # If specified, adds bias to input / output projection layers. Default: True.
            kdim = None,            # Total number of features for keys. Default: None (uses kdim=embed_dim).
            vdim = None,            # Total number of features for values. Default: None (uses vdim=embed_dim).
        )
        
        # Add non-linearity to attention.
        self.feedForwardLayers = nn.Sequential(
            # It is common to expand the signalEmbeddingDim
                # By a factor of 2-4 (small), 
                # By a factor of 4-8 (medium), 
                # By a factor of 8-16 (large).
                
            # Neural architecture: Layer 1.
            nn.Linear(signalEmbeddingDim, 64, bias = True),
            nn.ReLU(),
            nn.Dropout(0.2),
            ...
            nn.Linear(32, signalEmbeddingDim, bias = True),
            nn.ReLU(),
            nn.Dropout(0.2),
        )
        
        # Initialize the layer normalization.
        self.layerNorm_SA = nn.LayerNorm(signalEmbeddingDim, eps = 1E-10)
        self.layerNorm_FF = nn.LayerNorm(signalEmbeddingDim, eps = 1E-10)
        self.layerNorm_Final = nn.LayerNorm(signalEmbeddingDim, eps = 1E-10)
    ...
    def forward(self, inputData, allTrainingData = False):
        ...
        for layerInd in range(self.numLayers):
            ...
            selfAttentionData = self.selfAttentionBlock(inputData, allTrainingData)
            inputData = inputData + selfAttentionData
            
            # Combine the values from the feed-forward block.
            feedForwardData = self.feedForwardBlock(inputData) # Apply feed-forward block.
            inputData = inputData + feedForwardData
            
        # Final normalization after encoding.
        inputData = self.layerNorm_Final(inputData)
        
        return inputData
    
    def selfAttentionBlock(self, inputData, allTrainingData):
        # Apply self attention block to the signals.
        normInputData = self.layerNorm_SA(inputData)
        attentionOutput, attentionWeights = self.attentionMechanism(normInputData, normInputData, normInputData, 
    ...
    def feedForwardBlock(self, inputData):
        # Apply feed forward block to the signals.
        normInputData = self.layerNorm_FF(inputData)
        outputANN = self.feedForwardLayers(normInputData)
    ...
    def visualizeAttention(self, modelInd, batchInd  = 0):
        ...
        for plotInd in range(numPlots):
            ...
            im = ax.imshow(self.allAttentionWeights[0][0][batchInd, 0, :, :-1].detach().numpy(), 
                           cmap='RdBu', interpolation='nearest', animated=True, origin='lower', 
