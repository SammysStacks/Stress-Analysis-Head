...
class channelPositionalEncoding(signalEncoderModules):
    def __init__(self, maxSequenceLength=64):
        ...
        self.learnSignalPositions = self.learnModulePositionalEncoding(inChannel=1)
        self.unlearnSignalPositions = self.learnModulePositionalEncoding(inChannel=1)

    # ---------------------------------------------------------------------- #
    # -------------------- Learned Positional Encoding --------------------- #

    def addPositionalEncoding(self, inputData):
        return self.positionalEncoding(inputData, self.learnSignalPositions, addingData=1)

    def removePositionalEncoding(self, inputData):
        return self.positionalEncoding(inputData, self.unlearnSignalPositions, addingData=-1)
