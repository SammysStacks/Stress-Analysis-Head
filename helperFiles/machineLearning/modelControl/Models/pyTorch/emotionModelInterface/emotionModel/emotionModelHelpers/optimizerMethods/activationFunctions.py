...
class reversibleLinearSoftSign(reversibleInterface):
    ...
    def forward(self, x, linearModel, inverseFirst=True):
        ...
        x = self.inversePass(x) if inverseFirst else self.forwardPass(x)
        x = linearModel(x)  # Learn in the scaled domain.
        x = self.forwardPass(x) if inverseFirst else self.inversePass(x)
