...
class modelVisualizations(globalPlottingProtocols):
    ...
    def plotAllTrainingEvents(self, submodel, modelPipeline, lossDataLoader, trainingDate, currentEpoch):
        ...
        with torch.no_grad():
            ...
            reconstructedHealthProfile, compiledSignalEncoderLayerStates = model.reconstructPhysiologicalProfile(resampledSignalData)
            ...
            if self.accelerator.is_local_main_process:
                ...
                if submodel == modelConstants.signalEncoderModel:
                    ...
                    self.signalEncoderViz.modelFlow(dataTimes=resampledBiomarkerTimes, dataStates=compiledSignalEncoderLayerStates[:, 0], epoch=currentEpoch, saveFigureLocation="signalEncoding/", plotTitle="Data Flow within Model", batchInd=0, signalInd=0)
                    self.signalEncoderViz.modelPropagation3D(neuralEigenvalues=spatialEigenvalues, epoch=currentEpoch, degreesFlag=True, saveFigureLocation="signalEncoding/", plotTitle="Spatial Eigenvalues by Layer", batchInd=0, signalInd=0)
                    self.signalEncoderViz.modelPropagation3D(neuralEigenvalues=neuralEigenvalues, epoch=currentEpoch, degreesFlag=True, saveFigureLocation="signalEncoding/", plotTitle="Neural Eigenvalues by Layer", batchInd=0, signalInd=0)

                    # Plot the signal encoding training information.
                    self.signalEncoderViz.plotEigenValueLocations(specificEigenvalues[:, 0, allTrainingSignalMask[batchInd], :], testingEigenValues=specificEigenvalues[:, 0, allTestingSignalMask[batchInd], :], epoch=currentEpoch, signalInd=0, saveFigureLocation="signalEncoding/", plotTitle="Specific Eigenvalues on Circle")
                    self.signalEncoderViz.plotEigenValueLocations(sharedEigenvalues[:, 0, allTrainingSignalMask[batchInd], :], testingEigenValues=sharedEigenvalues[:, 0, allTestingSignalMask[batchInd], :], epoch=currentEpoch, signalInd=0, saveFigureLocation="signalEncoding/", plotTitle="Shared Eigenvalues on Circle")
                    ...
                    self.signalEncoderViz.plotEigenvalueAngles(sharedEigenvalues[:, 0, allTrainingSignalMask[batchInd], :], testingEigenValues=sharedEigenvalues[:, 0, allTestingSignalMask[batchInd], :], epoch=currentEpoch, degreesFlag=True, signalInd=0, saveFigureLocation="signalEncoding/", plotTitle="Shared Eigenvalue Angles")

                    self.signalEncoderViz.plotSignalEncodingStatePath(relativeTimes=None, compiledSignalEncoderLayerStates=np.expand_dims(spatialEigenvalues, axis=1), epoch=currentEpoch, hiddenLayers=0, saveFigureLocation="signalEncoding/", plotTitle="Spatial Eigenvalue Flow")
                    self.signalEncoderViz.plotSignalEncodingStatePath(relativeTimes=None, compiledSignalEncoderLayerStates=np.expand_dims(neuralEigenvalues, axis=1), epoch=currentEpoch, hiddenLayers=0, saveFigureLocation="signalEncoding/", plotTitle="Neural Eigenvalue Flow")
                    ...
                    if signalEncoderLayerTransforms.shape[0] != 0: self.signalEncoderViz.plotProfilePath(relativeTimes=resampledBiomarkerTimes, healthProfile=healthProfile, retrainingProfilePath=signalEncoderLayerTransforms[:, -1, :, signalInd, :], epoch=currentEpoch, saveFigureLocation="signalEncoding/", plotTitle="Generating Reconstructed Signal")
