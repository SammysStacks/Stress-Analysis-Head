...
class reversibleLieLayer(reversibleLieLayerInterface):
    ...
    def __init__(self, numSignals, sequenceLength, numLayers, activationMethod):
        ...
        initialMaxGivensAngle = self.getInverseAngleParams(torch.tensor(1e-3 * math.pi/180))
    ...
    def percentParamThresholding(self, layerInd, epoch):
        with torch.no_grad():
            ...
            elif 64 == self.sequenceLength: numParamsKeeping = self.numParams - epoch*12
            elif 32 == self.sequenceLength: numParamsKeeping = self.numParams - epoch*2
