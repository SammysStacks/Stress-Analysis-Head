...
class emotionModelWeights(convolutionalHelpers):
    ...
    def getInitialPhysiologicalProfile(numExperiments, encodedDimension):
        ...
        physiologicalProfile = nn.init.normal_(physiologicalProfile, mean=0, std=0.25)
    ...
    def physiologicalSmoothing(self):
        return nn.Sequential(
            self.convolutionalFilters_resNetBlocks(numResNets=3, numBlocks=2, numChannels=[1, 1], kernel_sizes=3, dilations=1, groups=1, strides=1, convType='conv1D', activationMethod="selu", numLayers=None, addBias=False),
            self.convolutionalFiltersBlocks(numBlocks=3, numChannels=[1, 1], kernel_sizes=3, dilations=1, groups=1, strides=1, convType='conv1D', activationMethod="selu", numLayers=None, addBias=False),
        )
