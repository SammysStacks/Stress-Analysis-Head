...
class emotionModelWeights(convolutionalHelpers):
    ...
    def physiologicalGeneration(self, numOutputFeatures):
        ...
        for i in range(max(0, numLayers - numUpSamples)): layers.append(self.linearModel(numInputFeatures=modelConstants.numEncodedWeights, numOutputFeatures=modelConstants.numEncodedWeights, activationMethod='selu', addBias=False));
        ...
        layers.append(self.convolutionalFilters_resNetBlocks(numResNets=numLayers, numBlocks=4, numChannels=[1, 1], kernel_sizes=3, dilations=1, groups=1, strides=1, convType='conv1D', activationMethod="selu", numLayers=None, addBias=False))
