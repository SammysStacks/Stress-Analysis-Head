...
class lossCalculations:
    ...
    def setEmotionActivityLossFunctions(self, activityClassWeights, emotionClassWeights):
        ...
        self.emotionCrossEntropyLoss = [torch.nn.CrossEntropyLoss(weight=emotionClassWeight, reduction='none', label_smoothing=1.0) for emotionClassWeight in emotionClassWeights]
    ...
    def calculateActivityLoss(self, predictedActivityProfile, allLabels, allLabelsMask):
        ...
        activityLosses = self.activityCrossEntropyLoss(predictedActivityProfile[activityDataMask], trueActivityLabels.long())
    ...
    def calculateEmotionLoss(self, predictedEmotionProfile, allLabels, allLabelsMask):
        ...
        numExperiments, numEmotions, encodedDimension = predictedEmotionProfile.shape
        assert self.emotionCrossEntropyLoss is not None

        # Find the boolean flags for the data involved in the loss calculation.
        emotionDataMask = self.dataInterface.getEmotionMasks(allLabelsMask, numEmotions=numEmotions)  # Dim: numExperiments, numEmotions
        emotionLosses = torch.zeros(numExperiments, numEmotions, device=predictedEmotionProfile.device)
        # predictedEmotionProfile: numExperiments, numEmotions, encodedDimension

        for emotionInd in range(numEmotions):
            emotionMask = self.dataInterface.getEmotionColumn(emotionDataMask, emotionInd)
            emotionLosses[emotionMask][emotionInd] = self.emotionCrossEntropyLoss[emotionInd](predictedEmotionProfile[emotionMask][emotionInd], allLabels[emotionMask][emotionInd].long())
