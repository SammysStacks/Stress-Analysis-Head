...
class optimizerMethods:
    ...
    def getModelParams(submodel, model):
        modelParams = [
            ...
            {'params': model.sharedSignalEncoderModel.parameters(), 'weight_decay': 0, 'lr': 1},
            {'params': (param for name, param in model.specificSignalEncoderModel.named_parameters() if "profileModel" not in name), 'weight_decay': 0, 'lr': 1},
            {'params': model.specificSignalEncoderModel.profileModel.parameters(), 'weight_decay': 1e-4, 'lr': 1},
        ]
    ...
    def addOptimizer(self, submodel, model):
        ...
        optimizer = self.setOptimizer(modelParams, lr=1e-2, weight_decay=0, optimizerType=self.userInputParams["optimizerType"])
    ...
    def getLearningRateScheduler(optimizer):
        ...
        schedulerOrder = [
            ...
            optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=2*numEpochCounts, eta_min=1e-2, last_epoch=-1),
        ]
