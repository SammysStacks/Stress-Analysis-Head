...
class optimizerMethods:
    ...
    def getModelParams(submodel, model):
        modelParams = [
            ...
            {'params': (param for name, param in model.specificSignalEncoderModel.named_parameters() if "jacobianParameter" in name), 'weight_decay': 1e-4, 'lr': 4e-4},
            {'params': (param for name, param in model.sharedSignalEncoderModel.named_parameters() if "jacobianParameter" in name), 'weight_decay': 1e-4, 'lr': 4e-4},
            ...
            {'params': (param for name, param in model.sharedSignalEncoderModel.named_parameters() if "activationFunction" in name), 'weight_decay': 1e-5, 'lr': 1e-4},
        ]
