...
class optimizerMethods:
    ...
    def getModelParams(self, submodel, model):
        modelParams = [
            ...
            {'params': (param for name, param in model.specificSignalEncoderModel.named_parameters() if "profileModel" not in name), 'weight_decay': 1e-6, 'lr': 1e-3},
            {'params': model.specificSignalEncoderModel.profileModel.parameters(), 'weight_decay': 1e-6, 'lr': self.userInputParams["learningRate"]},
        ]
