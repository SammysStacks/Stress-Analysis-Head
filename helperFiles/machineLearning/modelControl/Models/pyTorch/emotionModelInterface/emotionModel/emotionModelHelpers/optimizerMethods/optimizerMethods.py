...
class optimizerMethods:
    ...
    def getModelParams(submodel, model):
        modelParams = [
            ...
            {'params': (param for name, param in model.sharedSignalEncoderModel.named_parameters() if ("givensRotationParams" not in name and "healthGenerationModel" not in name and "fourierModel" not in name)), 'weight_decay': 1e-5, 'lr': 1e-4},  # 1e-5 - 5e-4
            {'params': (param for name, param in model.sharedSignalEncoderModel.named_parameters() if "givensRotationParams" in name), 'weight_decay': 1e-4, 'lr': modelConstants.userInputParams['reversibleLR']},  # 1e-4 - 0.1
