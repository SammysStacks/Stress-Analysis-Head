...
class optimizerMethods:
    ...
    def getModelParams(self, submodel, model):
        modelParams = [
            ...
            {'params': (param for name, param in model.sharedSignalEncoderModel.named_parameters() if ("givensRotationParams" not in name and "healthGenerationModel" not in name)), 'weight_decay': self.userInputParams['reversibleWD'], 'lr': 2e-4},  # 1e-5 - 5e-4
            ...
            {'params': (param for name, param in model.specificSignalEncoderModel.named_parameters() if ("givensRotationParams" not in name and "profileModel" not in name)), 'weight_decay': self.userInputParams['reversibleWD'], 'lr': 2e-4},  # 1e-2 - 1e2
