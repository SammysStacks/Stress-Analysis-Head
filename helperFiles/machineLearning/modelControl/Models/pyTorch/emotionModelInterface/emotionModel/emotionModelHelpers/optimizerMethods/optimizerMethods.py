...
class optimizerMethods:
    ...
    def addOptimizer(self, submodel, model):
        ...
        modelParams = [
            ...
            {'params': (param for name, param in model.named_parameters() if "activationFunction" in name), 'weight_decay': 1e-4, 'lr': 4e-4},
...
class CosineAnnealingLR_customized(optim.lr_scheduler.LRScheduler):
    ...
    def get_lr(self):
        ...
        if self.last_epoch <= self.numWarmupEpochs: return self.updateStep(multiplicativeFactor=self.warmupFactor, base_lrs=[max(self.absolute_min_lr, base_lr / (self.numWarmupEpochs - self.last_epoch + 1)) for base_lr in self.base_lrs])
