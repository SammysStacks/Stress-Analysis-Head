...
class optimizerMethods:
    ...
    def addOptimizer(self, submodel, model):
        ...
        modelParams = [
            ...
            {'params': (param for name, param in model.named_parameters() if "activationFunction" in name), 'weight_decay': 4e-5, 'lr': 4e-4},
            {'params': (param for name, param in model.named_parameters() if "jacobianParameter" in name), 'weight_decay': 1e-5, 'lr': 1e-4},
        ]

        if submodel == modelConstants.emotionModel:
            modelParams.extend([
                {'params': model.specificEmotionModel.subjectSpecificWeights, 'weight_decay': 1e-4, 'lr': 1e-3},
                {'params': model.specificEmotionModel.basicEmotionWeights, 'weight_decay': 1e-4, 'lr': 1e-3},
            ])
