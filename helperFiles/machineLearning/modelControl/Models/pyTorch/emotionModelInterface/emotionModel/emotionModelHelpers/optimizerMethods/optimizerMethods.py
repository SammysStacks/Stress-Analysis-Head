...
class optimizerMethods:
    ...
    def getModelParams(submodel, model):
        modelParams = [
            ...
            {'params': (param for name, param in model.specificSignalEncoderModel.named_parameters() if "profileModel" not in name), 'weight_decay': 1e-3, 'lr': 0.1},
    ...
    def getLearningRateScheduler(optimizer):
        ...
        numEpochCounts = 10*3  # 10 counts per epoch session.
