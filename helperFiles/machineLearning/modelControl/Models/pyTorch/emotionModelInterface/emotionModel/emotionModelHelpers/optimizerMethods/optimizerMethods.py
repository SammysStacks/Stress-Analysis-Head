...
class optimizerMethods:
    ...
    def getModelParams(self, submodel, model):
        modelParams = [
            ...
            {'params': model.sharedSignalEncoderModel.parameters(), 'weight_decay': 0, 'lr': 1e-2},  # 0.1 - 0.001
            {'params': (param for name, param in model.specificSignalEncoderModel.named_parameters() if "profileModel" not in name), 'weight_decay': 0, 'lr': 1e-2},  # 0.1 - 0.001
            {'params': model.specificSignalEncoderModel.profileModel.parameters(), 'weight_decay': 0, 'lr': 1e-1},  # 0.1 - 0.01
        ]
