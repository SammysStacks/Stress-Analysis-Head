...
class emotionModelHead(nn.Module):
    ...
    def addNewLayer(self):
        ...
        if numSpecificLayers % self.goldenRatio == 1: assert self.numModelLayers == (numSpecificLayers - 1)*self.goldenRatio + 1, f"The number of layers in the specific model ({numSpecificLayers}) does not match the number of layers in the model ({self.numModelLayers})."
