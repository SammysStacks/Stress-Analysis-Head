...
class emotionModelHead(nn.Module):
    ...
    def addNewLayer(self):
        ...
        if self.numModelLayers % self.goldenRatio == 0: assert numSpecificLayers == math.log2(self.numModelLayers - 1) + 1, f"The number of layers in the specific model ({numSpecificLayers}) does not match the number of layers in the model ({self.numModelLayers})."
