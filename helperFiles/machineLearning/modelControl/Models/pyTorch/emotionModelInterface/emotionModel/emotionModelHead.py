...
class emotionModelHead(nn.Module):
    ...
    def visualizeSignalEncoding(self, embeddedProfile, healthProfile, resampledSignalData, reconstructedSignalData, signalData, validDataMask):
        ...
        plt.plot(resampledBiomarkerTimes, healthProfile[firstBatchInd].clone().detach().cpu().numpy(), 'o-', color='tab:red', linewidth=0.1, label='Health Profile', alpha=2/3, markersize=2)
        ...
        plt.plot(validTimepoints, validReconstructedPoints, 'o-', color='tab:red', markersize=3, label='Reconstructed Signal', alpha=0.75, linewidth=0.1)
