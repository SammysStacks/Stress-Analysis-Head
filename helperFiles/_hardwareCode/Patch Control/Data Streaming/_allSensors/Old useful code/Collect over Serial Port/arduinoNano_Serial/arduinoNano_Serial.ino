...
int foreheadTemp;
int channelEOG_Vertical;
int channelEOG_Horizontal;
...
void loop() {  
    ...
    channelEOG_Vertical = analogReadFast(ADC0);     // Read the voltage value of A0 port (EOG Channel1)
    channelEOG_Horizontal = analogReadFast(ADC1);   // Read the voltage value of A1 port (EOG Channel2)
    ...
    sprintf(buffer, "%s.%s,%i,%i,%i,%i,%i,%i", String(currentSecond).c_str(), padZeros(currentMicros, 6).c_str(), 
            channelEOG_Vertical, channelEOG_Horizontal, redValue, irValue, foreheadTemp, galvanicSkinResponse);
