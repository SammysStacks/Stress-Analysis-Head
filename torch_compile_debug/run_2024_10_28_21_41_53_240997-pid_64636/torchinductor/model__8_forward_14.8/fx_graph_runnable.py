...
class Repro(torch.nn.Module):
    ...
    def forward(self, primals_1, primals_2, primals_3):
        ...
        mul_3 = torch.ops.aten.mul.Tensor(mul_2, primals_2);  mul_2 = None
        add_1 = torch.ops.aten.add.Tensor(mul_3, primals_3);  mul_3 = None
        ...
        div = torch.ops.aten.div.Tensor(primals_1, sub);  primals_1 = sub = None
        return (div, primals_2, primals_3)
...
def load_args(reader):
    ...
    reader.tensor(buf1, (), dtype=torch.float64, is_leaf=True)  # primals_2
    buf2 = reader.storage(None, 8, device=device(type='cuda', index=0), dtype_hint=torch.float64)
    reader.tensor(buf2, (), dtype=torch.float64, is_leaf=True)  # primals_3
