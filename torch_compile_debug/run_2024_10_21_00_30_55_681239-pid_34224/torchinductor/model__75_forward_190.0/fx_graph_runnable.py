...
class Repro(torch.nn.Module):
    ...
    def forward(self, primals_1, primals_2, primals_3, primals_4, primals_5):
        ...
        unsqueeze = torch.ops.aten.unsqueeze.default(primals_5, 3);  primals_5 = None
        ...
        view = torch.ops.aten.view.default(permute_2, [1, primals_4, 129]);  permute_2 = None
        ...
        view_2 = torch.ops.aten.view.default(bmm, [primals_4, 1, 1, 129]);  bmm = None
        permute_4 = torch.ops.aten.permute.default(view_2, [0, 2, 3, 1]);  view_2 = None
        view_3 = torch.ops.aten.view.default(permute_4, [primals_4, 1, 129]);  permute_4 = None
...
def load_args(reader):
    ...
    reader.symint(2530)  # primals_4
    buf3 = reader.storage(None, 1032*s0, device=device(type='cuda', index=0), dtype_hint=torch.float64)
    reader.tensor(buf3, (s0, 1, 129), dtype=torch.float64, is_leaf=True)  # primals_5
