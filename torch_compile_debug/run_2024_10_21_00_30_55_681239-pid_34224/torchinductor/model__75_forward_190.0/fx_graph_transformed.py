class GraphModule(torch.nn.Module):
    def forward(self, primals_1: "f64[1, 129, 129]", primals_2: "f64[1, 129, 129]", primals_3: "f64[129, 129]", primals_4: "Sym(s0)", primals_5: "f64[s0, 1, 129]"):
        ...
        unsqueeze: "f64[s0, 1, 129, 1]" = torch.ops.aten.unsqueeze.default(primals_5, 3);  primals_5 = None
        permute: "f64[s0, 1, 1, 129]" = torch.ops.aten.permute.default(unsqueeze, [0, 1, 3, 2]);  unsqueeze = None
        ...
        permute_2: "f64[s0, 129, 1, 1]" = torch.ops.aten.permute.default(permute, [0, 3, 1, 2]);  permute = None
        view: "f64[1, s0, 129]" = torch.ops.aten.reshape.default(permute_2, [1, primals_4, 129]);  permute_2 = None
        permute_3: "f64[129, 1, 129, 1]" = torch.ops.aten.permute.default(permute_1, [3, 1, 2, 0]);  permute_1 = None
        view_1: "f64[1, 129, 129]" = torch.ops.aten.reshape.default(permute_3, [1, 129, 129]);  permute_3 = None
        bmm: "f64[1, s0, 129]" = torch.ops.aten.bmm.default(view, view_1)
        view_2: "f64[s0, 1, 1, 129]" = torch.ops.aten.reshape.default(bmm, [primals_4, 1, 1, 129]);  bmm = None
        permute_4: "f64[s0, 1, 129, 1]" = torch.ops.aten.permute.default(view_2, [0, 2, 3, 1]);  view_2 = None
        view_3: "f64[s0, 1, 129]" = torch.ops.aten.reshape.default(permute_4, [primals_4, 1, 129]);  permute_4 = None
        permute_6: "f64[1, 129, s0]" = torch.ops.aten.permute.default(view, [0, 2, 1]);  view = None
        permute_7: "f64[1, 129, 129]" = torch.ops.aten.permute.default(view_1, [0, 2, 1]);  view_1 = None
        return [view_3, primals_2, permute_6, permute_7, primals_4]
