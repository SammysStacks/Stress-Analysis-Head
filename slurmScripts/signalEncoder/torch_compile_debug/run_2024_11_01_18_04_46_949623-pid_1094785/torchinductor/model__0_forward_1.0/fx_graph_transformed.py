class GraphModule(torch.nn.Module):
    def forward(self, primals_1: "f64[11, 64, 120, 2]", primals_2: "f32[11, 64, 3]", primals_3: "f32[11, 2]", primals_4: "f64[60, 128]"):
        ...
        convert_element_type: "i32[11, 64, 3]" = torch.ops.prims.convert_element_type.default(primals_2, torch.int32);  primals_2 = None
        convert_element_type_1: "i32[11, 2]" = torch.ops.prims.convert_element_type.default(primals_3, torch.int32);  primals_3 = None
        ...
        full_default: "f64[11, 6, 128]" = torch.ops.aten.full.default([11, 6, 128], 0, dtype = torch.float64, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
        ...
        full_default_1: "f64[11, 32, 128]" = torch.ops.aten.full.default([11, 32, 128], 0, dtype = torch.float64, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
        ...
        full_default_2: "f64[11, 128]" = torch.ops.aten.full.default([11, 128], 0, dtype = torch.float64, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
        ...
        select: "i32[11, 64]" = torch.ops.aten.select.int(convert_element_type, 2, 0)
        ...
        expand: "i64[11, 64, 120]" = torch.ops.aten.expand.default(iota, [11, 64, 120]);  iota = None
        ...
        unsqueeze: "i32[11, 64, 1]" = torch.ops.aten.unsqueeze.default(select, -1);  select = None
        lt: "b8[11, 64, 120]" = torch.ops.aten.lt.Tensor(expand, unsqueeze);  expand = unsqueeze = None
        
         # File: /central/groups/GaoGroup/ssolomon/Stress-Analysis-Head/helperFiles/machineLearning/modelControl/Models/pyTorch/emotionModelInterface/emotionModel/emotionModelHelpers/emotionDataInterface.py:151 in getSignalIdentifierData, code: return signalIdentifiers[:, :, channelInd]
        select_1: "i32[11, 64]" = torch.ops.aten.select.int(convert_element_type, 2, 2);  convert_element_type = None
        ...
        select_2: "i32[11]" = torch.ops.aten.select.int(select_1, 1, 0);  select_1 = None
        ...
        index: "f64[11, 128]" = torch.ops.aten.index.Tensor(primals_4, [select_2]);  primals_4 = None
