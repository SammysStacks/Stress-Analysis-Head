...
class Repro(torch.nn.Module):
    ...
    def forward(self, primals_1, primals_2, primals_3, primals_4):
        ...
        full_default = torch.ops.aten.full.default([10, 6, 128], 0, dtype = torch.float64, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
        full_default_1 = torch.ops.aten.full.default([10, 32, 128], 0, dtype = torch.float64, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
        full_default_2 = torch.ops.aten.full.default([10, 128], 0, dtype = torch.float64, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
        ...
        expand = torch.ops.aten.expand.default(iota, [10, 64, 120]);  iota = None
...
def load_args(reader):
    buf0 = reader.storage(None, 1228800, device=device(type='cuda', index=0), dtype_hint=torch.float64)
    reader.tensor(buf0, (10, 64, 120, 2), dtype=torch.float64, is_leaf=True)  # primals_1
    buf1 = reader.storage(None, 640000, device=device(type='cuda', index=0))
    reader.tensor(buf1, (10, 64, 3), (16000, 250, 2), storage_offset=240, is_leaf=True)  # primals_2
    reader.tensor(buf1, (10, 2), (16000, 2), storage_offset=246, is_leaf=True)  # primals_3
